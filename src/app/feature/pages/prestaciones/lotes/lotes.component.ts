...
export class LotesComponent extends BaseWorkflowClass implements OnDestroy {
    ...
    columns: [
      {
        ...
        type: "html",
        cssClass:
          "px-1 py-3 border-bottom border-light align-middle pr-1 text-nowrap",
        width: "150px",
      },
      {
        key: "asuntoFormat",
        ...
        type: "html",
        width: "100%",
        cssClass: "px-1 py-3 border-bottom border-light align-middle",
      },
      {
        key: "fechaFormat",
        header: "",
        ...
        align: "right",
        cssClass:
          "px-3 py-3 border-bottom border-light align-middle font-weight-500 text-nowrap",
      },
    ],
    actions: [
      {
        ...
        tooltip: "Rechazar",
        buttonClass:
          "btn btn-sm btn-pastel-icon-circle pastel-danger shadow-sm ml-1 tooltip-action",
      },
      {
        name: "procesar",
        icon: "fa-cogs",
        tooltip: "Procesar",
        buttonClass:
          "btn btn-sm btn-pastel-icon-circle pastel-warning shadow-sm ml-1 tooltip-action",
      },
    ],
  };
  ...
  public rawLotes: any[] = [
    {
      id: "1",
      periodo: "ENE-2024",
      directiva: "Incremento Presidencial 2023",
      cantidad: "15,420",
      estatus: "Procesado",
      fecha: "Hoy, 09:12",
    },
    {
      id: "2",
      periodo: "DIC-2023",
      directiva: "Ajuste Salarial Ordinario",
      cantidad: "14,800",
      estatus: "Validado",
      fecha: "Ayer, 16:45",
    },
  ...
  constructor(
    ...
    super(
      apiService,
      layoutService,
      "Principal / Prestaciones: Procesos en Lote",
    );
    ...
    switch (actionName) {
      case "aprobar":
        this.modalService
          ...
          .result.then(
            (result) => {
              if (result === "confirm") this.ejecutarAccion("aprobar", row);
            },
            () => {},
          );
        break;
      case "rechazar":
        this.modalService
          ...
          .result.then(
            (result) => {
              if (result === "confirm") this.ejecutarAccion("rechazar", row);
            },
            () => {},
          );
        break;
  ...
  public abrirModalConfirmarCalculo(): void {
    if (this.fechaDesde || this.fechaHasta) {
      if (!this.fechaDesde || !this.fechaHasta) {
        this.utilService.AlertMini(
          "top-end",
          "error",
          "Debe especificar ambas fechas (Desde y Hasta) para el rango",
          3000,
        );
        return;
      }
      const desdeStr = this.utilService.parseAndNormalizeDate(this.fechaDesde);
      const hastaStr = this.utilService.parseAndNormalizeDate(this.fechaHasta);
    ...
    this.modalService
      .open(this.modalConfirmarCalculo, {
        centered: true,
        size: "md",
        windowClass: "pastel-modal",
      })
  ...
  public abrirModalConfirmarAccion(tipo: string, row?: any): void {
    ...
    this.modalService
      .open(this.modalConfirmarAccion, {
        centered: true,
        size: "md",
        windowClass: "pastel-modal",
      })
  ...
  public getTabIcon(name: string): string {
    ...
    if (str.includes("COMPONENTE") || str.includes("PROCESO"))
      return "fa-shield-alt";
    if (str.includes("FIDEICOMISO") || str.includes("APROBADO"))
  ...
  private notifyCompletion(msg: any) {
    ...
    this.zone.run(() => {
      ...
      if (typeof newContent === "string") {
        textToType = newContent;
      } else if (newContent && newContent.message) {
        textToType = newContent.message;
      } else {
        textToType = JSON.stringify(newContent, null, 2);
      }
      ...
      this.trackId = this.utilService.GenerarId();
      this.selectDirectiva = "0";
      this.selectSituacion = "201";
      this.selectComponente = "99";
      this.selectGrado = "99";
      this.fechaProceso = "";
      this.fechaDesde = "";
      this.fechaHasta = "";
  ...
  public prepararIndices(): void {
    ...
    const selectedDir = this.lstDirectivas.find(
      (d) => String(d.id) === String(this.selectDirectiva),
    );
    const cicloStr = selectedDir ? String(selectedDir.id) : "03MAR2026";
    ...
    const fnx = {
      ...
      nombre: "NOMINA DE PRESTACIONES SOCIALES",
      autor: "Sandra",
      ciclo: `DIR#${cicloStr}`,
      fecha: this.fechaHasta,
      grado: this.selectGrado,
      componente: this.selectComponente,
      situacion: this.selectSituacion,
      directiva_id: this.selectDirectiva,
    };
  ...
  public abrirModalGenerar(): void {
    this.modalService.open(this.modalGenerar, {
      centered: true,
      size: "md",
      windowClass: "pastel-modal",
    });
    ...
    this.apiService.post("crud", payload).subscribe({
      next: (data: any) => {
        if (data.Cuerpo && data.Cuerpo.length > 0) {
          this.lstDirectivas = data.Cuerpo.sort(
      ...
      error: (error) => {
        this.isLoadingData = false;
        console.error("Error al consultar directivas:", error);
      },
    });
  }
