...
export class LoginService {
  ...
  getLogin(user: string, clave: string): Observable<IToken> {
    ...
    return from(
      ...
      switchMap((encodeDeviceContext) => {
        ...
        return this.apiService.post<IToken>(
          "wusuario/loginV2",
          usuario,
          undefined,
          headers,
  ...
  private detectarComponenteMilitar(usuario: any): string {
    ...
    for (const field of fieldsToSearch) {
      if (field) {
        ...
        if (
          fieldUpper.includes("EJÉRCITO") ||
          fieldUpper.includes("EJERCITO")
        ) {
          return "EJERCITO";
        } else if (fieldUpper.includes("ARMADA")) {
          return "ARMADA";
        } else if (
          fieldUpper.includes("AVIACIÓN") ||
          fieldUpper.includes("AVIACION")
        ) {
          return "AVIACION";
        } else if (
          fieldUpper.includes("GUARDIA") ||
          fieldUpper.includes("GNB") ||
          fieldUpper.includes("G.N.")
        ) {
          return "GUARDIA";
        }
      }
  ...
  obtenerPrivilegiosMenu(menu: string): any {
    let Prv: any = {};
    ...
    if (lst && Array.isArray(lst)) {
      lst.forEach((e: any) => {
        if (e.Privilegios && e.Privilegios.length > 0) {
          const prefijo = e.url
            ? e.url.replace(/\//g, "").substring(0, 3).toLowerCase()
            : "";
          e.Privilegios.forEach((p: any) => {
            p.codigo = prefijo + (p.metodo ? p.metodo.toLowerCase() : "");
          });
          Prv[e.url] = e.Privilegios;
        }
      });
    }
    return Prv;
  ...
  protected obenterAplicacion(itk: string) {
    if (!this.Token || !this.Token.Usuario) {
      console.error(
        "LoginService: No se puede obtener aplicación sin datos de usuario decodificados",
      );
      return;
    }
    ...
    this.apiService.post("crud", this.xAPI).subscribe(
      async (data: any) => {
        try {
          ...
          let menu = await this.consolidarAplicaciones(data[0].Aplicacion).Rol
            .Menu;
          ...
          this.componenteService.initComponentes().subscribe({
            error: (err) =>
              console.error("Error al inicializar componentes:", err),
          });
          this.estatusBeneficiarioService.initEstatus().subscribe({
            error: (err) =>
              console.error(
                "Error al inicializar estatus de beneficiarios:",
                err,
              ),
          });
          this.cargarMenu();
      ...
      app.Rol.Menu.forEach((itemMenu: any) => {
        ...
        if (!menuMap.has(menuKey)) {
          ...
          itemMenu.SubMenu.forEach((nuevoSub: any) => {
            const subExistente = menuExistente.SubMenu.find(
              (s: any) => s.url === nuevoSub.url,
            );
            ...
            if (!subExistente) {
              ...
              subExistente.Privilegios?.forEach((p: any) =>
                privilegiosMap.set(p.metodo, p),
              );
              // Inyectamos privilegios del nuevo rol sin sobreescribir los existentes
              nuevoSub.Privilegios?.forEach((p: any) =>
                privilegiosMap.set(p.metodo, p),
              );
