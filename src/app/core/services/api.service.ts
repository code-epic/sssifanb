...
export class ApiService {
  ...
  public get<T>(
    ...
    console.log(this._resolveUrl(endpoint));
  ...
  public put<T>(
    ...
    return this._http.put<T>(this._resolveUrl(endpoint), body, {
      params,
      headers,
    });
  }
  ...
  public delete<T>(
    ...
    return this._http.delete<T>(this._resolveUrl(endpoint), {
      params,
      headers,
    });
  }
  ...
  public async postStream<T>(
    ...
    try {
      ...
      console.log(
        `[ApiService] Respuesta Recibida: ${response.status} ${response.statusText}`,
      );
      ...
      if (!response.body) {
        throw new Error(
          "El cuerpo de la respuesta está vacío (no hay stream).",
        );
      }
      ...
      while (true) {
        ...
        if (done) {
          console.log(
            `[ApiService] Stream finalizado. Total filas: ${rowCount}`,
          );
          break;
        }
        ...
        for (const line of lines) {
          ...
          if (trimmedLine) {
            try {
              ...
              console.error(
                "[ApiService] Error parseando JSON en línea:",
                trimmedLine,
                e,
              );
            }
